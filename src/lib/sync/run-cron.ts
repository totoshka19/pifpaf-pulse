 ...
 * часть.
 *
 * ЗАМЕРЕНО в задаче 9, не выбрано на глаз. Десять просроченных рилсов,
 * из них три с настоящими данными фикстуры (то есть с тремя попытками
 * скачать обложку — потолок THUMBNAILS_PER_TICK) и семь `unavailable`:
 *
 ...
 *
 * Десять давали 6,2 с при потолке Netlify в 10 с — меньше двукратного
 * запаса, и запас этот тратится ещё до прода: замер идёт на `APIFY_MOCK=1`,
 * где `getRun` и `getDatasetItems` отвечают мгновенно, а живой Apify
 * добавит по круговой задержке на каждый. Отсюда пять.
 *
 * Что показал замер сверх ожидаемого: дорога не обложка, а КРУГОВАЯ
 * ЗАДЕРЖКА ДО БАЗЫ. Тик, собравший пять рилсов вообще без единой попытки
 * скачать обложку (все `unavailable`), занял 4091 мс — почти столько же,
 * сколько тик с тремя скачиваниями. Значит стоимость линейна по числу
 * рилсов в батче, а не по числу картинок, и уменьшение батча — верный
 * рычаг. Замер сделан с машины разработчика, у которой до Neon идёт
 * публичный интернет; функция Netlify живёт ближе к базе, так что число
 * здесь заведомо пессимистичное.
 *
 * При 96 тиках в сутки это потолок в 480 рилсов, то есть очередь «кому пора»
...
export async function runCronTick(
  ...
  try {
    ...
    // НАХОДКА 1 (раунд правок 1/5, задача 8). failRuns закрывает sync_runs
    // безусловно, но её апдейт reels загорожен условием `syncStatus =
    // 'pending'` — верным условием для ЕЁ основного вызывающего
    // (collectFinishedRuns на СЛЕДУЮЩЕМ тике, когда рилс уже был переведён
    // в pending прошлым УСПЕШНЫМ запуском). Здесь же startRun падает ДО
    // того единственного места ниже по этой функции, которое ставит
    // pending, — рилсы всё ещё в том статусе, с которым пришли (обычно
    // 'ok'), условие не совпадает, и failRuns настоящих рилсов не трогает.
    //
    // Без отдельного сдвига next_sync_at здесь рилсы остаются максимально
    // просроченными (тем же значением, из-за которого их выбрал dueReels),
    // и следующий тик — через 15 минут — выбирает их же снова. При
    // УСТОЙЧИВОЙ причине падения (отозванный токен, переименованный актор)
    // это 4 тика в час × до CRON_BATCH_SIZE рилсов = до 960 резервов
    // бюджета в сутки, хотя Apify ни разу не был вызван по-настоящему —
    // tryReserve списывает квоту за прогоны, которых не было. Месячный
    // потолок в 1850 выжигается меньше чем за двое суток, причём именно
    // тогда, когда что-то и так сломано.
    //
    // syncStatus НЕ трогаем: данные рилса никуда не делись, мы просто не
    // смогли сходить за свежими — это не 'failed' по существу, а честное
    // 'ok' с устаревшими цифрами. failRuns и её условие не менялись —
    // для collectFinishedRuns оно по-прежнему верное.
    //
    // Покрыто тестом (раунд правок 2/5): маркерный шорткод MOCK_THROW_START
    // (src/lib/apify/client.ts) заставляет startRun бросить внутри isMock(),
    // не трогая боевой путь — см. run-cron.integration.test.ts, describe
    // «run-cron — ветки отказа Apify».
    await db
      .update(reels)
      .set({ nextSyncAt: new Date(now.getTime() + FAILED_RETRY_MS) })
      .where(
        inArray(
          reels.id,
