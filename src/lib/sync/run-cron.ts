...
export async function runCronTick(
  ...
  try {
    ...
    //
    // ПРОБЕЛ ПОКРЫТИЯ, ОСОЗНАННЫЙ. Прямого интеграционного теста на эту
    // catch-ветку нет. Мок startRun (src/lib/apify/client.ts) решает по
    // isMock() и НЕ умеет падать ни при каком состоянии БД — url и
    // shortcode в reels оба NOT NULL, значит даже испорченной строкой эту
    // ветку не спровоцировать так же, как остальные тесты файла вызывают
    // свои сценарии (данными, не шпионами). Единственные способы вызвать
    // настоящее исключение — временно увести isMock() на живую ветку
    // (сетевой вызов к реальному Apify — запрещено заданием независимо от
    // валидности токена) или завести в наборе первый vi.mock ради одного
    // теста (сознательно отклонено координатором, чтобы не плодить стиль,
    // которого в интеграционном наборе проекта больше нигде нет). Отложено
    // координатором как принятый пробел покрытия.
    await db
      .update(reels)
