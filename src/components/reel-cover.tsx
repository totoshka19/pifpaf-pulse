  ...
  return (
    ...
    >
      {broken ? (
        ...
        >
          {/*
            Цвет буквы ПРИБИТ, а не взят из `--ink`: градиент над ней светлый
            при любой теме, значит и буква обязана оставаться тёмной. Через
            `--ink` она в тёмной теме становилась светлой и пропадала на
            светлом фоне — плейсхолдер выглядел пустым прямоугольником.
            Тот же случай и та же починка, что у буквы в `ui/avatar.tsx`;
            правило целиком записано в шапке `globals.css`.

            Видно только когда обложка не загрузилась, поэтому глазами
            дефект не попадался — нашёлся чтением кода.
          */}
          <span className="text-5xl font-semibold text-[#0f172a]/30 select-none">
            {letter}
          </span>
        </div>
      ) : (
        ...
        // ничего взамен.
        // `group-hover` срабатывает только там, где предок объявил `group`, —
        // это карточка ленты и больше нигде. В таблице, в топ-3 и на экране
        // рилса зума не будет, и так и задумано: приближение обложки это
        // приглашение кликнуть, а кликать оттуда либо некуда, либо ведёт
        // другой элемент. Связь неявная, поэтому записана здесь.
        // Контейнер выше — `overflow-hidden` со скруглением, так что
        // увеличенная картинка обрезается по краю карточки.
        <img
          src={src}
          alt={alt}
