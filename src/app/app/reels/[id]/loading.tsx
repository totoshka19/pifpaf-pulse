  ...
  return (
    <div className="flex animate-pulse flex-col gap-6">
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
        <div className="aspect-[9/16] w-40 shrink-0 self-center rounded-[var(--radius)] bg-white/60 sm:self-start" />
        ...
        <div className="flex flex-1 flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {Array.from({ length: 4 }, (_, i) => (
              <div key={i} className="h-12 rounded bg-white/60" />
            ))}
          </div>

          <div className="h-10 w-36 rounded-xl bg-white/60" />
        </div>
      </div>

      <div className="h-80 rounded-[var(--radius)] bg-white/60" />
      <div className="h-40 rounded-[var(--radius)] bg-white/60" />
    </div>
  )
}
