...
describe('dueReels — кого крон берёт в работу', () => {
  ...
  beforeAll(async () => {
    ...
    // Порядок вставки НАРОЧНО обратен ожидаемому порядку выдачи — сначала
    // наименее просроченный (DueNow), потом самый просроченный (DueOld).
    // Это не косметика, а находка мутационной проверки (срез 7, задача 6,
    // шаг 5): на маленькой тестовой таблице планировщик Postgres выбирает
    // Seq Scan и без ORDER BY возвращает строки в физическом порядке —
    // на свежей таблице это практически порядок вставки (подтверждено
    // через EXPLAIN). Со старым порядком вставки (Old → Now) тест ниже
    // оставался бы зелёным, даже если `.orderBy()` в dueReels убрать
    // целиком: совпадение порядка вставки с ожидаемым ответом маскировало
    // отсутствие сортировки. Обратный порядок вставки это совпадение
    // исключает — без ORDER BY Seq Scan вернёт DueNow первым, и тест
    // обязан покраснеть. Не «упрощать» обратно на Old → Now.
    await mk('DueNow', ago(5), 'ok') // просрочен недавно — вставлен ПЕРВЫМ
    await mk('DueOld', ago(120), 'ok') // просрочен сильнее всех — вставлен ВТОРЫМ
  ...
  it('самый просроченный идёт первым', async () => {
    ...
    // и DueNull сняты с расписания (см. тесты выше) — значит toEqual по
    // всему списку сравнивает точную последовательность из двух элементов,
    // а не гадает по одному индексу.
    expect(mine.map((r) => r.shortcode)).toEqual(['DueOld', 'DueNow'])
  })
