...
|---|---|---|
| Тестов на обработку: 8 | **11** | Добавлены остальные замеренные размеры (1179×2096, 720×1280), проверка запаса до переполнения `integer` в колонке `bytes`, обрезанный JPEG (обрыв скачивания). |
| Тестов на скачивание: 8 | **13** | Добавлены `403` (протухшая подпись Instagram), ответ без `content-type`, файл ровно на границе лимита, разные варианты `image/*`, проверка что до сети **вообще не дошло** при чужой схеме. |
| — | **+ `src/lib/images/live.integration.test.ts`** | Юнит-тесты подменяют сеть и генерируют картинки — они пройдут, даже если CDN начнёт отдавать HTML или `sharp` разучится читать JPEG от Instagram. Добавлена сквозная проверка на трёх настоящих обложках: скачивание + обработка целиком. Падение с внятным сообщением «фикстуры протухли» подскажет, что нужна свежая выгрузка, а не правка кода. |

### Проверка тестов на зубы

| Что ломали | Результат |
|---|---|
| Убрали `withoutEnlargement` из `resize` | **Упал 1 тест.** Апскейл маленькой картинки ловится. |
| Убрали проверку схемы URL в `fetchImage` | **Упало 2 теста.** Единственное место среза с настоящим риском безопасности защищено: `file:///etc/passwd` и `data:` не доходят до сети. |
...
- [x] **Шаг 4: Тесты зелёные** — `npm test -- images/fetch`, ожидаем `8 passed`
...
- [x] **Шаг 1: Убедиться, что сборка на Linux не упала**
...
- [x] `npm test` — зелёный, покрыты обработка и скачивание картинок
- [x] `npm run test:db` — зелёный, покрыто хранение и идемпотентность
- [x] `npm run build` — проходит **и локально, и на Netlify** (`sharp` собрался)
- [x] Обложка отдаётся с нашего домена как `image/webp` — **проверено на проде**
- [x] Вес обложки в 4–7 раз меньше оригинала — 70 КБ вместо 338 КБ
- [x] Заголовок `private, max-age=31536000, immutable` выставляется
- [ ] **Кеш браузера проверен глазами в DevTools** — заголовок стоит, но фактическое
      поведение (`(disk cache)` на втором открытии) не смотрел: для этого нужен
      браузер, а не `curl`. Закрывается в срезе 5 вместе с лентой.
- [x] Чужая обложка отдаётся как `404`, гостю `401`
- [x] Мёртвая ссылка не роняет приём метрик: `sync_status` остаётся `ok`
- [ ] **`<ReelCover>` увиден в работе** — компонент написан и собирается, но нигде
      не отрисован: лента это срез 5. Плейсхолдер проверен только чтением кода.
- [x] Дозаливка работает — проверена на подставном рилсе: нашла, скачала за 1602 мс,
      сохранила 70 КБ; повторный прогон сообщает «дозаливать нечего»
- [x] Замеры в `PLAN.md` заменены на фактические

### Проверка прода (`npm run verify:prod`)

| Что | Результат |
|---|---|
| Режим Apify | **живой** — `apify_run_id = exzn4ROMFMOiaAj8c`, не `mock:` |
| Прогон | `ok` за 14 с, `@aabbigeyl`, 722 424 просмотра |
| Обложка | `200`, `image/webp`, сигнатура `RIFF....WEBP` |
| Вес | **70 КБ** |
| Кеш | `private, max-age=31536000, immutable` |
| **`sharp` на Linux** | **собрался** — WebP по сигнатуре тому доказательство |

Расход Apify за все проверки среза: **3 результата из 1500**.

> Способ определить режим прода без доступа к настройкам Netlify: мок пишет
> в `sync_runs.apify_run_id` префикс `mock:`, живой Apify — настоящий id.
> База у прода и разработки одна, так что читать можно прямо из неё.
