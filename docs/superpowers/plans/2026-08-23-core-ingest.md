...
## Статус: СРЕЗ ЗАКРЫТ — все 9 задач выполнены

`npm test` → **109 passed**. `npm run test:db` → **21 passed** на живой базе.
`npm run e2e:reels` → **24 проверки** на запущенном дев-сервере.
`npm run build` → проходит.

### Сквозная проверка потока (`npm run e2e:reels`)

Клиент на Node, а не `curl`: в Windows-консоли кириллица в теле запроса портится,
и можно получить как ложную ошибку, так и ложный успех (выяснено в срезе 1).

| Что | Результат |
|---|---|
| Вставка ссылки | `202`, статус `pending` |
| Карточка после вставки | статус `ok`, один снапшот, `@aabbigeyl`, 662 153 просмотра, дата публикации |
| Повторный запрос карточки | снапшот по-прежнему один |
| Дубль ссылки | `409` |
| Чужой домен, пустая строка | `400` с человеческим текстом |
| Без сессии | `401` |
| Лента | оба рилса с последними метриками |
| `?sort=DROP TABLE` | `200` — белый список отсекает мусор |
| Чужая карточка / чужое удаление | `404`, неотличимо от несуществующей |
| Лента другого пользователя | пусто |
| Удаление своего | `200`, каскад уносит снапшоты |

### Найденный баг: мок засорял счётчик Apify

Обнаружен не тестом, а взглядом на данные после прогона: `apify_usage` показывал
4 результата, хотя работали на фикстурах и кредиты не тратились.

`POST /api/reels` резервировал бюджет **независимо от режима**. Разработка ведётся
на моках сотнями вызовов — за несколько дней предохранитель решил бы, что лимит
выбран, и отказал бы в синхронизации настоящих рилсов на проде. То есть защита
от перерасхода вызвала бы ровно ту аварию, от которой оберегает.

Исправлено в месте вызова (`!isMock() && !(await tryReserve(1))`), а не внутри
`tryReserve`: сама функция обязана оставаться честной — интеграционные тесты
вызывают её напрямую и проверяют гонку. Счётчик сброшен.
...
- [x] **Шаг 1: Написать запрос сырым SQL**
...
- [x] **Шаг 2: Подключить к route handler**
...
- [x] **Шаг 4: Сверить с тем, что показывает Instagram**
...
- [x] `npm test` — 109 зелёных: расписание, бюджет, клиент Apify, ответы API
- [x] `npm run test:db` — 21 на живой базе: гонка бюджета, правило снапшотов
- [x] `npm run e2e:reels` — 24 проверки потока на дев-сервере
- [x] `npm run build` — проходит
- [x] Вставка ссылки создаёт рилс и первый снапшот с реальными данными
- [x] Повторная вставка той же ссылки даёт `409`
- [x] Чужой рилс отдаётся как `404`, неотличимо от несуществующего
- [x] Приватный или удалённый рилс становится `unavailable`, а не висит в `pending`
- [x] **Живой Apify сверен** — см. ниже
- [x] Расход за срез: **1 результат из 1500**, при выделенных на разработку 300
- [ ] **На проде живой режим не включён** — в Netlify нужно добавить `APIFY_TOKEN`
      и выставить `APIFY_MOCK=0`. Без этого прод продолжит отдавать фикстуры.

### Сверка с живым Apify (`npm run verify:live`)

Сверялись на том же рилсе, что лежит в фикстурах: если живые метрики окажутся
больше снятых часом раньше — это подтвердит разом, что схема не изменилась,
данные настоящие и правило «снапшот только при изменении» сработает на росте.

| Параметр | Фикстура (19:05 UTC) | Живой вызов (20:05 UTC) |
|---|---|---|
| Просмотры | 662 153 | **694 917** (+32 764) |
| Лайки | 20 769 | 22 020 |
| Комментарии | 536 | 584 |
| Автор, дата публикации | совпали | совпали |

**Прогон занял 19 секунд** (в `sync_runs` записано 18). Это главный результат
сверки: лимит функции на Netlify — 10 секунд, то есть синхронный
`run-sync-get-dataset-items` оборвался бы **вдвое раньше**, чем Apify ответил.
Архитектура с опросом карточки подтверждена замером, а не рассуждением.

Побочно подтвердилось предсказание из разбора фикстур: `videoDuration` пришёл
как `10.377869`, в базе лежит `10.38` — колонка `numeric(6,2)` округлила.
